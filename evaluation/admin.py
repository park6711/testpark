...
import csv
from .models import EvaluationNo, FixFee, Complain, Satisfy, Evaluation, YearEvaluation
...
class FixFeeAdmin(admin.ModelAdmin):
    ...
    readonly_fields = [
        ...
        'get_due_status',
        'get_days_overdue',
        'get_payment_delay_days',
        'is_overdue',
        'is_early_payment',
        'calculate_late_fee',
        'get_payment_summary_display'
    ]

    fieldsets = (
        ('기본 정보', {
            'fields': ('no', 'noCompany', 'get_company_name', 'created_at', 'updated_at')
        }),
        ('납부 정보', {
            'fields': ('dateToDo', 'dateDeposit', 'nDeposit', 'bDeposit')
        }),
        ('상태 정보', {
            'fields': ('get_payment_status', 'get_due_status', 'get_days_overdue', 'get_payment_delay_days')
        }),
        ('추가 정보', {
            'fields': ('is_overdue', 'is_early_payment', 'calculate_late_fee'),
            'classes': ('collapse',)
        }),
        ('메모', {
            'fields': ('sMemo',)
        ...
        ('요약 정보', {
            'fields': ('get_payment_summary_display',),
            'classes': ('collapse',)
        })
    )

    ordering = ['-dateToDo', '-no']
    list_per_page = 25

    actions = ['mark_as_paid', 'mark_as_unpaid', 'export_to_csv', 'send_overdue_notice']
    ...
    get_status_badge.short_description = '납부상태'

    def get_due_status_badge(self, obj):
        """기한 상태 뱃지"""
        due_status = obj.get_due_status()

        if '연체' in due_status:
            color = 'red'
        elif '임박' in due_status or '당일' in due_status:
            color = 'orange'
        elif '완료' in due_status:
            color = 'green'
        else:
            color = 'blue'
    ...
    get_due_status_badge.short_description = '기한상태'

    def get_overdue_info(self, obj):
        """연체 정보"""
        if obj.is_overdue():
            overdue_days = obj.get_days_overdue()
            late_fee = obj.calculate_late_fee()
            return format_html(
                '<span style="color: red;">{}일 연체<br>연체료: {:,}원</span>',
                overdue_days,
                late_fee
            )
        elif obj.is_early_payment():
            return format_html('<span style="color: blue;">조기납부</span>')
        else:
            return '-'
    get_overdue_info.short_description = '연체정보'

    def get_payment_summary_display(self, obj):
        """납부 요약 정보"""
        ...
        return format_html(
            ...
            summary['due_date'],
            summary['payment_date'] or '미납부',
            summary['amount'],
            summary['status'],
            summary['due_status'],
            '예' if summary['is_overdue'] else '아니오',
            summary['overdue_days'],
            '예' if summary['is_early'] else '아니오'
        )
    get_payment_summary_display.short_description = '납부요약'
    ...
    def mark_as_paid(self, request, queryset):
        ...
        for obj in queryset:
            if not obj.bDeposit:
                obj.mark_as_paid(payment_date=today)
                updated += 1

        self.message_user(request, f"{updated}개 고정비가 완납 처리되었습니다.")
    mark_as_paid.short_description = "선택된 고정비를 완납 처리"
    ...
    def mark_as_unpaid(self, request, queryset):
        ...
        for obj in queryset:
            if obj.bDeposit:
                obj.bDeposit = False
                obj.dateDeposit = None
                obj.nDeposit = 0
                obj.save()
                updated += 1

        self.message_user(request, f"{updated}개 고정비가 미납 처리되었습니다.")
    mark_as_unpaid.short_description = "선택된 고정비를 미납 처리"

    def export_to_csv(self, request, queryset):
        """CSV 내보내기"""
        response = HttpResponse(content_type='text/csv')
        response['Content-Disposition'] = 'attachment; filename="fix_fees.csv"'
        response.write('\ufeff')  # UTF-8 BOM

        writer = csv.writer(response)
        writer.writerow([
            '고정비ID', '업체ID', '업체명', '납부기준일', '납부일자', '실납부액',
            '완납여부', '납부상태', '기한상태', '연체일수', '연체료', '비고', '생성일시'
        ])

        for obj in queryset:
            writer.writerow([
                obj.no, obj.noCompany, obj.get_company_name(), obj.dateToDo,
                obj.dateDeposit or '', obj.nDeposit, '완납' if obj.bDeposit else '미납',
                obj.get_payment_status(), obj.get_due_status(), obj.get_days_overdue(),
                obj.calculate_late_fee(), obj.sMemo, obj.created_at
            ])

        return response
    export_to_csv.short_description = "선택된 고정비를 CSV로 내보내기"
    ...
    def send_overdue_notice(self, request, queryset):
        ...
        for obj in queryset:
            if obj.is_overdue():
                # 실제로는 SMS나 이메일 발송 로직 구현
                overdue_count += 1

        self.message_user(request, f"{overdue_count}개 업체에 연체 알림이 발송되었습니다.")
    send_overdue_notice.short_description = "연체 업체에 알림 발송"
