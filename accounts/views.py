...
class LoginView(View):
    ...
    def get(self, request):
        ...
        if request.user.is_authenticated:
            return redirect('demo:home')  # 업체 사용자 메인 페이지

        # 스텝 세션이 있는 경우 스텝 페이지로 리다이렉트
        if 'staff_user' in request.session:
            return redirect('staff:staff_list')  # 스텝 메인 페이지
...
class VerifyCodeView(View):
    ...
    def _handle_normal_verification(self, request, auth_session):
        ...
        # 계정 변경 모드인지 확인하여 팝업창 닫기 또는 리다이렉트
        return self._handle_login_redirect(request, 'company', user.name)
    ...
    def _handle_staff_verification(self, request, auth_session):
        ...
        # 계정 변경 모드인지 확인하여 팝업창 닫기 또는 리다이렉트
        return self._handle_login_redirect(request, 'staff', staff.sName)
...
class SetSwitchModeView(View):
    """계정 변경 모드 설정 (AJAX)"""

    def post(self, request):
        try:
            # 계정 변경 모드 플래그 설정
            request.session['is_switch_account'] = True

            # 네이버 로그아웃 후 로그인 URL 생성
            logout_url = naver_auth.get_logout_and_login_url('company')
        ...
        try:
            ...
            if request.user.is_authenticated:
                return JsonResponse({
                    'logged_in': True,
                    'user_type': 'company',
                    'redirect_url': '/demo/',  # demo:home URL
                    'user_name': request.user.name
                })

            # 스텝 사용자 로그인 확인
            if 'staff_user' in request.session:
                staff_info = request.session['staff_user']
                return JsonResponse({
                    'logged_in': True,
                    'user_type': 'staff',
                    'redirect_url': '/staff/',  # staff:staff_list URL
                    'user_name': staff_info.get('name', 'Unknown')
                })
...
class NaverCallbackView(View):
    ...
    def get(self, request):
        ...
        # state 검증 (계정 변경 모드에서는 우회)
        is_switch_mode = request.session.get('is_switch_account', False)
        session_state = request.session.get('naver_state')

        print(f"[DEBUG] State 검증 - session: {session_state}, callback: {state}, switch_mode: {is_switch_mode}")

        if not is_switch_mode and session_state != state:
            print(f"[ERROR] State 불일치 (일반 모드) - session: {session_state}, callback: {state}")
            messages.error(request, "잘못된 요청입니다.")
            return redirect('accounts:login')
        elif is_switch_mode:
            print(f"[DEBUG] 계정 변경 모드 - state 검증 우회")

        try:
            # 🚀 완전 자동화된 사용자 타입 감지 및 처리
            return self._auto_detect_user_type_and_process(request, code, state)
        except Exception as e:
            print(f"[ERROR] 콜백 처리 중 예외 발생: {str(e)}")
            import traceback
            traceback.print_exc()
    ...
    def _handle_company_login(self, request, code, state):
        ...
        if existing_user:
            ...
            # 계정 변경 모드인지 확인하여 팝업창 닫기 또는 리다이렉트
            return self._handle_login_redirect(request, 'company', existing_user.name)
    ...
    def _handle_staff_login(self, request, code, state):
        ...
        if existing_staff:
            ...
            # 계정 변경 모드인지 확인하여 팝업창 닫기 또는 리다이렉트
            return self._handle_login_redirect(request, 'staff', existing_staff.sName)
    ...
    def _handle_login_redirect(self, request, user_type, user_name):
        """로그인 완료 후 리다이렉트 처리 (계정 변경 모드 고려)"""
        # 계정 변경 모드인지 확인
        is_switch_mode = request.session.get('is_switch_account', False)

        if is_switch_mode:
            # 계정 변경 모드 플래그 제거
            request.session.pop('is_switch_account', None)
            # 팝업창 자동 닫기
            return render(request, 'accounts/close_popup.html', {
                'success_message': f"✅ {user_name}님으로 로그인 완료!"
            })
        else:
            # 일반 로그인 - 타입에 따라 목적지 결정
            if user_type == 'staff':
                return redirect('staff:staff_list')
            else:
                return redirect('demo:home')

    def _auto_detect_user_type_and_process(self, request, code, state):
        """네이버 로그인 후 자동으로 사용자 타입 감지 및 처리"""
        print(f"[DEBUG] 자동 타입 감지 시작")
        ...
        if ':' in state:
            ...
            original_state = state
            login_type = 'company'
        print(f"[DEBUG] 사용자 선택 로그인 타입: {login_type}")
        ...
        naver_email = user_info['email']
        naver_name = user_info['name']
        print(f"[DEBUG] 네이버 인증 성공: {naver_email} ({naver_id})")

        # 2. 사용자가 선택한 타입에 따라 우선순위 조정
        if login_type == 'staff':
            # 스텝 로그인 선택 시: 스텝 먼저 검색
            print(f"[DEBUG] 스텝 로그인 선택 - 스텝 우선 검색")

            # 2-1. 기존 연동 스텝 사용자 확인
            existing_staff_user = Staff.objects.filter(sNaverID0=naver_id).first()
            if existing_staff_user:
                print(f"[DEBUG] 기존 스텝 사용자 발견: {existing_staff_user.sName}")
                request.session['staff_user'] = {
                    'no': existing_staff_user.no,
                    'name': existing_staff_user.sName,
                    'email': existing_staff_user.sNaverID,
                    'team': existing_staff_user.sTeam,
                    'naver_id': existing_staff_user.sNaverID0,
                }
                messages.success(request, f"환영합니다, {existing_staff_user.sName}님! (스텝 로그인)")
                return self._handle_login_redirect(request, 'staff', existing_staff_user.sName)

            # 2-2. 기존 연동 업체 사용자 확인 (스텝이 없을 경우만)
            existing_company_user = CustomUser.objects.filter(naver_id=naver_id).first()
            if existing_company_user:
                print(f"[DEBUG] 기존 업체 사용자 발견하지만 스텝 로그인 선택: {existing_company_user.email}")
                # 스텝 로그인을 선택했지만 업체만 연동된 경우, 스텝 신규 연동 확인
                staff_user_by_email = Staff.objects.filter(sNaverID=naver_email).first()
                if staff_user_by_email:
                    print(f"[DEBUG] 스텝 신규 연동 대상 발견: {staff_user_by_email.sName}")
                    return self._handle_new_staff_linking(request, user_info, staff_user_by_email)
                else:
                    # 스텝 테이블에도 없으면 업체로 로그인
                    login(request, existing_company_user)
                    messages.success(request, f"환영합니다, {existing_company_user.name}님! (업체 계정으로 로그인)")
                    jandi_webhook.send_login_success(existing_company_user.name, existing_company_user.email)
                    return self._handle_login_redirect(request, 'company', existing_company_user.name)
        else:
            # 업체 로그인 선택 시: 업체 먼저 검색
            print(f"[DEBUG] 업체 로그인 선택 - 업체 우선 검색")

            # 2-1. 기존 연동 업체 사용자 확인
            existing_company_user = CustomUser.objects.filter(naver_id=naver_id).first()
            if existing_company_user:
                print(f"[DEBUG] 기존 업체 사용자 발견: {existing_company_user.email}")
                login(request, existing_company_user)
                messages.success(request, f"환영합니다, {existing_company_user.name}님!")
                jandi_webhook.send_login_success(existing_company_user.name, existing_company_user.email)
                return self._handle_login_redirect(request, 'company', existing_company_user.name)

            # 2-2. 기존 연동 스텝 사용자 확인 (업체가 없을 경우만)
            existing_staff_user = Staff.objects.filter(sNaverID0=naver_id).first()
            if existing_staff_user:
                print(f"[DEBUG] 기존 스텝 사용자 발견하지만 업체 로그인 선택: {existing_staff_user.sName}")
                # 업체 로그인을 선택했지만 스텝만 연동된 경우, 업체 신규 연동 확인
                company_user_by_email = CustomUser.objects.filter(email=naver_email).first()
                if company_user_by_email:
                    print(f"[DEBUG] 업체 신규 연동 대상 발견: {company_user_by_email.email}")
                    return self._handle_new_company_linking(request, user_info, company_user_by_email)
                else:
                    # 업체 테이블에도 없으면 스텝으로 로그인
                    request.session['staff_user'] = {
                        'no': existing_staff_user.no,
                        'name': existing_staff_user.sName,
                        'email': existing_staff_user.sNaverID,
                    ...
                    messages.success(request, f"환영합니다, {existing_staff_user.sName}님! (스텝 계정으로 로그인)")
                    return self._handle_login_redirect(request, 'staff', existing_staff_user.sName)

        # 4. 신규 연동 처리 - 업체 이메일 검색
        company_user_by_email = CustomUser.objects.filter(email=naver_email).first()
        if company_user_by_email:
            print(f"[DEBUG] 업체 신규 연동 대상: {company_user_by_email.email}")
            return self._handle_new_company_linking(request, user_info, company_user_by_email)

        # 5. 신규 연동 처리 - 스텝 이메일 검색
        staff_user_by_email = Staff.objects.filter(sNaverID=naver_email).first()
        if staff_user_by_email:
            print(f"[DEBUG] 스텝 신규 연동 대상: {staff_user_by_email.sName}")
            return self._handle_new_staff_linking(request, user_info, staff_user_by_email)
    ...
    def _handle_new_company_linking(self, request, user_info, company_user):
        """신규 업체 사용자 네이버 연동 처리 (인증번호 발송)"""
        print(f"[DEBUG] 업체 신규 연동 시작: {company_user.email}")

        # 인증번호 생성 및 발송
        auth_code = company_user.generate_auth_code()

        if jandi_webhook.send_auth_code(company_user.email, auth_code):
            # 인증 세션 생성
            session_key = request.session.session_key
            if not session_key:
                request.session.save()
            ...
            print(f"[DEBUG] 업체 인증번호 발송 성공")
            messages.info(request, "인증번호가 잔디로 발송되었습니다. 확인해주세요.")
            return redirect('accounts:verify_code')
        else:
            print(f"[ERROR] 업체 인증번호 발송 실패")
            messages.error(request, "인증번호 발송에 실패했습니다. 다시 시도해주세요.")
            return redirect('accounts:login')

    def _handle_new_staff_linking(self, request, user_info, staff_user):
        """신규 스텝 사용자 네이버 연동 처리 (인증번호 발송)"""
        print(f"[DEBUG] 스텝 신규 연동 시작: {staff_user.sName}")

        # 인증번호 생성 및 발송
        auth_code = staff_user.generate_auth_code()

        if jandi_webhook.send_auth_code(staff_user.sNaverID, auth_code):
            # 스텝용 인증 세션 생성
            session_key = request.session.session_key
            if not session_key:
                request.session.save()
            ...
            AuthSession.create_session(
                ...
                auth_code=auth_code,
                login_type='staff',
                staff_email=staff_user.sNaverID
            )

            print(f"[DEBUG] 스텝 인증번호 발송 성공")
            messages.info(request, f"스텝 인증번호가 잔디로 발송되었습니다. ({staff_user.sName}님)")
            return redirect('accounts:verify_code')
        else:
            print(f"[ERROR] 스텝 인증번호 발송 실패")
            messages.error(request, "스텝 인증번호 발송에 실패했습니다. 다시 시도해주세요.")
